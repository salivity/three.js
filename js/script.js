...
document.addEventListener("DOMContentLoaded", () => {
    ...
    videos.forEach(video => {
        video.setAttribute("poster", "/three.js/images/video/poster/default.svg")
    });
    ...
    if (articlesContainer) {
        fetch("/three.js/js/articles.json").then(res => res.json()).then(data => {
    ...
    const remapArticles = (data) => {
        return data.map((item) => {
            return { id: item[0], title: item[1], description: item[2], url: `/three.js/article/${kebabCase(item[1])}`, open_graph: item[3] }
        })
    }
    ...
    function renderList(items, container, paginatorId, page, isSearch) {
        ...
        pageItems.forEach(item => {
            ...
            if(item.open_graph === true){
                ...
                imageTop.style.backgroundImage = `url(/three.js/images/open-graph/article/${item.id}.webp)`
                ...
                imageBottom.src = `/three.js/images/open-graph/article/${item.id}.webp`
